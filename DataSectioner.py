...
for trip in my_list:
    ...
    print("processing user: ",user_id, ", trip: ", trip_id)
    ...
    df_user = df_user.iloc[30:]
    df_user.drop(df_user.tail(30).index, inplace=True)
    ...
    for i in range(int(df_user.shape[0]/10)):
        ...
        if counts.shape[0] != 1:
            for val in range(i*10, i*10+10):
                drop_multimodal.append(df_user.index[val])
